...
class GeneticScheduleOptimizer {
  ...
  double _evaluateWorkPatternAlignment(Schedule schedule) {
    final peakHours = workPatterns['peakHours'] as List<int>? ?? [9, 10, 11, 14, 15];
    ...
    for (final task in schedule.taskOrder) {
      if (task.dueDate != null) {
        ...
        if (timeUntilDeadline > 0) {
          ...
          adherenceScore += max(0.0, 1.0 + (timeUntilDeadline / 24)); // Penalty for overdue
        }
      }
  ...
  Schedule _tournamentSelection(List<Schedule> population, List<double> fitnessScores) {
  ...
  List<String> _generateOptimizationTips(Schedule schedule) {
    ...
    final highPriorityTasks = schedule.taskOrder
        .where((t) => t.priority == TaskPriority.high || t.priority == TaskPriority.critical)
        .length;
    
    if (highPriorityTasks > schedule.taskOrder.length * 0.3) {
      tips.add('Consider breaking down high-priority tasks into smaller chunks');
    }
    ...
    tips.add('Schedule your most challenging tasks during your peak performance hours');
  ...
  List<String> _identifyScheduleRisks(Schedule schedule) {
    ...
    final totalMinutes = schedule.taskOrder.fold<int>(0, (sum, task) => sum + task.estimatedMinutes);
    final workDays = (totalMinutes / (8 * 60)).ceil();
    
    if (workDays > 5) {
      risks.add('Schedule spans multiple weeks - consider prioritizing critical tasks');
    }
    
    // Check for deadline conflicts
    final tasksWithDeadlines = schedule.taskOrder.where((t) => t.dueDate != null);
    for (final task in tasksWithDeadlines) {
      if (task.dueDate!.isBefore(DateTime.now().add(Duration(days: 1)))) {
