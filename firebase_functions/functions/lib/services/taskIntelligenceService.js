...
exports.taskIntelligenceService = exports.TaskIntelligenceService = void 0;
class TaskIntelligenceService {
     ...
     */
    calculateConfidence(taskInput, aiResult) {
        var _a;
        let confidence = 0.5; // Base confidence
        // Title quality (longer, more descriptive titles get higher confidence)
        if (taskInput.title.length > 10)
            confidence += 0.1;
        if (taskInput.title.length > 25)
            confidence += 0.1;
        // Description quality
        if (taskInput.description && taskInput.description.length > 20)
            confidence += 0.15;
        if (taskInput.description && taskInput.description.length > 100)
            confidence += 0.15;
        // User context availability
        if ((_a = taskInput.userContext) === null || _a === void 0 ? void 0 : _a.uid)
            confidence += 0.1;
        // AI processing success indicators
        if (aiResult.tags && aiResult.tags.length > 0)
            confidence += 0.1;
        if (aiResult.optimizationTips && aiResult.optimizationTips.length > 0)
            confidence += 0.1;
        if (aiResult.suggestedTimeSlots && aiResult.suggestedTimeSlots.length > 0)
            confidence += 0.05;
        // Category specificity
        if (taskInput.category !== 'general')
            confidence += 0.05;
        // Priority specificity
        if (taskInput.priority !== 'medium')
            confidence += 0.05;
     ...
     */
    getTimeSlotRecommendations(category) {
        const timeSlotMap = {
            'planning': ['morning'],
     ...
     */
    async updateTaskIntelligence(taskId, actualDuration, userFeedback) {
        try {
     ...
     */
    async getIntelligenceSummary(userId) {
        try {
...
exports.TaskIntelligenceService = TaskIntelligenceService;
exports.taskIntelligenceService = new TaskIntelligenceService();
//# sourceMappingURL=taskIntelligenceService.js.map